...
가져오기는 누락되거나 오래되었거나 기하학적으로 잘못된 스냅샷을 거부한다. 성공하면 검수 완료 후보가 곧바로 일반 플레이용 `scenes.json`에 승격되고, 모바일에서 너무 작은 클릭 영역은 폴리곤 비율을 유지하며 최소 44 CSS 픽셀로 보정된다. 일반 방문자는 `dev=1`을 사용하지 않는 한 탐색 화면만 본다.
