...
The queue shows only scenes that still need human confirmation, including existing frontier scenes, so completed work does not remain mixed into the pending list. Edit mode always shows editable polygons. Click-test mode hides them by default and provides `영역 보기` only as a diagnostic overlay. `npm run production:status` reports every active candidate, confirmed alternative frontier, and frontier still blocked on human annotation or route reconciliation. Four-or-more-route scenes are never selected automatically without a separate rarity approval.
